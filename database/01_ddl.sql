-- =============================================================================
-- GESTION TECNOLOGICA FISEI - BASE DE DATOS ORACLE
-- ARCHIVO 01: DDL (DATA DEFINITION LANGUAGE)
-- Define la estructura de las tablas, restricciones, secuencias, triggers y procedimientos.
-- =============================================================================

-- =============================================================================
-- DOCUMENTACION DE TABLAS Y SUS FUNCIONES
-- =============================================================================
-- 1. DEPARTAMENTO: Define las áreas administrativas u organizativas (ej. Software, Electrónica).
-- 2. UBICACION: Define las dependencias físicas (laboratorios, aulas) asignadas a un departamento.
-- 3. ROL: Define los roles de seguridad que controlan los accesos en la aplicación.
-- 4. CATEGORIA: Catálogo de agrupaciones para organizar los artículos tecnológicos.
-- 5. ESTADO: Catálogo unificado para el manejo de estados de artículos, préstamos, mantenimientos, etc.
-- 6. USUARIO: Almacena la información de contacto y credenciales de acceso vinculadas a un rol.
-- 7. ARTICULO: Almacena los bienes e insumos del inventario con sus atributos de serie, stock y valor.
-- 8. PRESTAMO: Cabecera que registra la transacción de préstamo, usuario solicitante y rango de fechas.
-- 9. DETALLE_PRESTAMO: Tabla intermedia de muchos a muchos que detalla los artículos incluidos en un préstamo.
-- 10. MOVIMIENTO: Historial de traslados físicos de los equipos entre ubicaciones.
-- 11. MANTENIMIENTO: Registro de actividades de soporte técnico preventivo o correctivo por artículo.
-- 12. NOTIFICACION: Alerta automática sobre préstamos vencidos, devoluciones o mantenimientos.
-- 13. AUDITORIA: Registra de manera automatizada las acciones de inserción, edición y eliminación.
-- =============================================================================

SET DEFINE OFF;

-- ---------------------------------------------------------
-- 1. DEPARTAMENTO
-- ---------------------------------------------------------
CREATE TABLE departamento (
  id_dep        NUMBER,
  nom_dep       VARCHAR2(120) NOT NULL,
  des_dep       VARCHAR2(300),
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_departamento PRIMARY KEY (id_dep),
  CONSTRAINT uq_departamento_nom UNIQUE (nom_dep),
  CONSTRAINT ck_departamento_nom CHECK (TRIM(nom_dep) IS NOT NULL)
);

-- ---------------------------------------------------------
-- 2. UBICACION
-- ---------------------------------------------------------
CREATE TABLE ubicacion (
  id_ubi        NUMBER,
  nom_ubi       VARCHAR2(120) NOT NULL,
  des_ubi       VARCHAR2(300),
  id_dep        NUMBER NOT NULL,
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_ubicacion PRIMARY KEY (id_ubi),
  CONSTRAINT uq_ubicacion_nom_dep UNIQUE (nom_ubi, id_dep),
  CONSTRAINT ck_ubicacion_nom CHECK (TRIM(nom_ubi) IS NOT NULL),
  CONSTRAINT fk_ubicacion_departamento FOREIGN KEY (id_dep)
    REFERENCES departamento (id_dep)
);

-- ---------------------------------------------------------
-- 3. ROL
-- ---------------------------------------------------------
CREATE TABLE rol (
  id_rol        NUMBER,
  nom_rol       VARCHAR2(80) NOT NULL,
  des_rol       VARCHAR2(300),
  est_rol       NUMBER(1) DEFAULT 1 NOT NULL,
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_rol PRIMARY KEY (id_rol),
  CONSTRAINT uq_rol_nom UNIQUE (nom_rol),
  CONSTRAINT ck_rol_estado CHECK (est_rol IN (0, 1))
);

-- ---------------------------------------------------------
-- 4. CATEGORIA
-- ---------------------------------------------------------
CREATE TABLE categoria (
  id_cat        NUMBER,
  nom_cat       VARCHAR2(120) NOT NULL,
  des_cat       VARCHAR2(300),
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_categoria PRIMARY KEY (id_cat),
  CONSTRAINT uq_categoria_nom UNIQUE (nom_cat),
  CONSTRAINT ck_categoria_nom CHECK (TRIM(nom_cat) IS NOT NULL)
);

-- ---------------------------------------------------------
-- 5. ESTADO
-- ---------------------------------------------------------
CREATE TABLE estado (
  id_est        NUMBER,
  nom_est       VARCHAR2(80) NOT NULL,
  des_est       VARCHAR2(300),
  tipo_est      VARCHAR2(40) NOT NULL,
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_estado PRIMARY KEY (id_est),
  CONSTRAINT uq_estado_nom_tipo UNIQUE (nom_est, tipo_est),
  CONSTRAINT ck_estado_tipo CHECK (
    tipo_est IN ('ACTIVO', 'ARTICULO', 'PRESTAMO', 'MOVIMIENTO', 'MANTENIMIENTO', 'NOTIFICACION')
  )
);

-- ---------------------------------------------------------
-- 6. USUARIO
-- ---------------------------------------------------------
CREATE TABLE usuario (
  id_usr        NUMBER,
  nom_usr       VARCHAR2(120) NOT NULL,
  ape_usr       VARCHAR2(120) NOT NULL,
  cor_usr       VARCHAR2(180) NOT NULL,
  tel_usr       VARCHAR2(30),
  usu_login     VARCHAR2(80) NOT NULL,
  pass_hash     VARCHAR2(255) NOT NULL,
  est_usr       NUMBER(1) DEFAULT 1 NOT NULL,
  id_rol        NUMBER NOT NULL,
  id_dep        NUMBER,
  id_ubi        NUMBER,
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_usuario PRIMARY KEY (id_usr),
  CONSTRAINT uq_usuario_correo UNIQUE (cor_usr),
  CONSTRAINT uq_usuario_login UNIQUE (usu_login),
  CONSTRAINT ck_usuario_estado CHECK (est_usr IN (0, 1)),
  CONSTRAINT fk_usuario_rol FOREIGN KEY (id_rol)
    REFERENCES rol (id_rol),
  CONSTRAINT fk_usuario_departamento FOREIGN KEY (id_dep)
    REFERENCES departamento (id_dep),
  CONSTRAINT fk_usuario_ubicacion FOREIGN KEY (id_ubi)
    REFERENCES ubicacion (id_ubi)
);

-- ---------------------------------------------------------
-- 7. ARTICULO
-- ---------------------------------------------------------
CREATE TABLE articulo (
  id_art        NUMBER,
  nom_art       VARCHAR2(180) NOT NULL,
  des_art       VARCHAR2(500),
  ser_art       VARCHAR2(100),
  mar_art       VARCHAR2(80),
  mod_art       VARCHAR2(80),
  can_art       NUMBER DEFAULT 1 NOT NULL,
  val_art       NUMBER(12,2) DEFAULT 0 NOT NULL,
  est_art       NUMBER(1) DEFAULT 1 NOT NULL,
  id_cat        NUMBER NOT NULL,
  id_est        NUMBER NOT NULL,
  id_dep        NUMBER,
  id_ubi        NUMBER,
  fec_registro  DATE DEFAULT SYSDATE NOT NULL,
  CONSTRAINT pk_articulo PRIMARY KEY (id_art),
  CONSTRAINT uq_articulo_serial UNIQUE (ser_art),
  CONSTRAINT ck_articulo_cantidad CHECK (can_art >= 0),
  CONSTRAINT ck_articulo_valor CHECK (val_art >= 0),
  CONSTRAINT ck_articulo_estado CHECK (est_art IN (0, 1)),
  CONSTRAINT fk_articulo_categoria FOREIGN KEY (id_cat)
    REFERENCES categoria (id_cat),
  CONSTRAINT fk_articulo_estado FOREIGN KEY (id_est)
    REFERENCES estado (id_est),
  CONSTRAINT fk_articulo_departamento FOREIGN KEY (id_dep)
    REFERENCES departamento (id_dep),
  CONSTRAINT fk_articulo_ubicacion FOREIGN KEY (id_ubi)
    REFERENCES ubicacion (id_ubi)
);

-- ---------------------------------------------------------
-- 8. PRESTAMO
-- ---------------------------------------------------------
CREATE TABLE prestamo (
  id_pres       NUMBER,
  fec_pres      DATE DEFAULT SYSDATE NOT NULL,
  fec_entrega   DATE,
  fec_devolucion DATE,
  obs_pres      VARCHAR2(500),
  est_pres      NUMBER(1) DEFAULT 1 NOT NULL,
  id_usr        NUMBER NOT NULL,
  id_est        NUMBER NOT NULL,
  CONSTRAINT pk_prestamo PRIMARY KEY (id_pres),
  CONSTRAINT ck_prestamo_estado CHECK (est_pres IN (0, 1)),
  CONSTRAINT ck_prestamo_fechas CHECK (fec_devolucion IS NULL OR fec_devolucion >= fec_pres),
  CONSTRAINT fk_prestamo_usuario FOREIGN KEY (id_usr)
    REFERENCES usuario (id_usr),
  CONSTRAINT fk_prestamo_estado FOREIGN KEY (id_est)
    REFERENCES estado (id_est)
);

-- ---------------------------------------------------------
-- 9. DETALLE_PRESTAMO
-- ---------------------------------------------------------
CREATE TABLE detalle_prestamo (
  id_pres      NUMBER NOT NULL,
  id_art       NUMBER NOT NULL,
  can_pre      NUMBER DEFAULT 1 NOT NULL,
  CONSTRAINT pk_detalle_prestamo PRIMARY KEY (id_pres, id_art),
  CONSTRAINT ck_detalle_prestamo_cantidad CHECK (can_pre > 0),
  CONSTRAINT fk_detalle_prestamo_prestamo FOREIGN KEY (id_pres)
    REFERENCES prestamo (id_pres)
    ON DELETE CASCADE,
  CONSTRAINT fk_detalle_prestamo_articulo FOREIGN KEY (id_art)
    REFERENCES articulo (id_art)
);

-- ---------------------------------------------------------
-- 10. MOVIMIENTO
-- ---------------------------------------------------------
CREATE TABLE movimiento (
  id_mov        NUMBER,
  tip_mov       VARCHAR2(40) NOT NULL,
  fec_mov       DATE DEFAULT SYSDATE NOT NULL,
  obs_mov       VARCHAR2(500),
  id_art        NUMBER NOT NULL,
  id_usr        NUMBER NOT NULL,
  id_ubi_ori    NUMBER,
  id_ubi_des    NUMBER,
  CONSTRAINT pk_movimiento PRIMARY KEY (id_mov),
  CONSTRAINT ck_movimiento_tipo CHECK (tip_mov IN ('INGRESO', 'SALIDA', 'TRASLADO', 'BAJA', 'AJUSTE', 'PRESTAMO')),
  CONSTRAINT fk_movimiento_articulo FOREIGN KEY (id_art)
    REFERENCES articulo (id_art),
  CONSTRAINT fk_movimiento_usuario FOREIGN KEY (id_usr)
    REFERENCES usuario (id_usr),
  CONSTRAINT fk_movimiento_ubi_ori FOREIGN KEY (id_ubi_ori)
    REFERENCES ubicacion (id_ubi),
  CONSTRAINT fk_movimiento_ubi_des FOREIGN KEY (id_ubi_des)
    REFERENCES ubicacion (id_ubi),
  CONSTRAINT ck_movimiento_origen_destino CHECK (id_ubi_ori IS NULL OR id_ubi_des IS NULL OR id_ubi_ori <> id_ubi_des)
);

-- ---------------------------------------------------------
-- 11. MANTENIMIENTO
-- ---------------------------------------------------------
CREATE TABLE mantenimiento (
  id_man        NUMBER,
  des_man       VARCHAR2(300) NOT NULL,
  tip_man       VARCHAR2(60) NOT NULL,
  fec_ini       DATE DEFAULT SYSDATE NOT NULL,
  fec_fin       DATE,
  obs_man       VARCHAR2(500),
  est_man       NUMBER(1) DEFAULT 1 NOT NULL,
  id_art        NUMBER NOT NULL,
  id_usr        NUMBER NOT NULL,
  id_est        NUMBER NOT NULL,
  CONSTRAINT pk_mantenimiento PRIMARY KEY (id_man),
  CONSTRAINT ck_mantenimiento_estado CHECK (est_man IN (0, 1)),
  CONSTRAINT ck_mantenimiento_fechas CHECK (fec_fin IS NULL OR fec_fin >= fec_ini),
  CONSTRAINT fk_mantenimiento_articulo FOREIGN KEY (id_art)
    REFERENCES articulo (id_art),
  CONSTRAINT fk_mantenimiento_usuario FOREIGN KEY (id_usr)
    REFERENCES usuario (id_usr),
  CONSTRAINT fk_mantenimiento_estado FOREIGN KEY (id_est)
    REFERENCES estado (id_est)
);

-- ---------------------------------------------------------
-- 12. NOTIFICACION
-- ---------------------------------------------------------
CREATE TABLE notificacion (
  id_not        NUMBER,
  tit_not       VARCHAR2(120) NOT NULL,
  men_not       VARCHAR2(1000) NOT NULL,
  fec_not       DATE DEFAULT SYSDATE NOT NULL,
  est_not       NUMBER(1) DEFAULT 1 NOT NULL,
  id_usr        NUMBER NOT NULL,
  CONSTRAINT pk_notificacion PRIMARY KEY (id_not),
  CONSTRAINT ck_notificacion_estado CHECK (est_not IN (0, 1)),
  CONSTRAINT fk_notificacion_usuario FOREIGN KEY (id_usr)
    REFERENCES usuario (id_usr)
    ON DELETE CASCADE
);

-- ---------------------------------------------------------
-- 13. AUDITORIA
-- ---------------------------------------------------------
CREATE TABLE auditoria (
  id_aud        NUMBER,
  nom_tabla     VARCHAR2(80) NOT NULL,
  id_registro   VARCHAR2(80),
  nom_accion    VARCHAR2(20) NOT NULL,
  nom_usuario   VARCHAR2(80) NOT NULL,
  fec_aud       DATE DEFAULT SYSDATE NOT NULL,
  des_aud       VARCHAR2(1000),
  CONSTRAINT pk_auditoria PRIMARY KEY (id_aud),
  CONSTRAINT ck_auditoria_accion CHECK (nom_accion IN ('INSERT', 'UPDATE', 'DELETE'))
);

-- ---------------------------------------------------------
-- SECUENCIAS
-- ---------------------------------------------------------
CREATE SEQUENCE SEQ_ARTICULO START WITH 4 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_DEPARTAMENTO START WITH 4 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_UBICACION START WITH 4 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_ROL START WITH 5 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_CATEGORIA START WITH 3 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_ESTADO START WITH 8 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_USUARIO START WITH 4 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_PRESTAMO START WITH 3 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_MOVIMIENTO START WITH 3 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_MANTENIMIENTO START WITH 3 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_NOTIFICACION START WITH 2 INCREMENT BY 1 NOCACHE NOCYCLE;
CREATE SEQUENCE SEQ_AUDITORIA START WITH 2 INCREMENT BY 1 NOCACHE NOCYCLE;

-- ---------------------------------------------------------
-- TRIGGERS DE AUTO-INCREMENTO
-- ---------------------------------------------------------
CREATE OR REPLACE TRIGGER TRG_DEPARTAMENTO_ID
BEFORE INSERT ON DEPARTAMENTO
FOR EACH ROW
BEGIN
  IF :NEW.ID_DEP IS NULL THEN
    SELECT SEQ_DEPARTAMENTO.NEXTVAL INTO :NEW.ID_DEP FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_UBICACION_ID
BEFORE INSERT ON UBICACION
FOR EACH ROW
BEGIN
  IF :NEW.ID_UBI IS NULL THEN
    SELECT SEQ_UBICACION.NEXTVAL INTO :NEW.ID_UBI FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_ROL_ID
BEFORE INSERT ON ROL
FOR EACH ROW
BEGIN
  IF :NEW.ID_ROL IS NULL THEN
    SELECT SEQ_ROL.NEXTVAL INTO :NEW.ID_ROL FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_CATEGORIA_ID
BEFORE INSERT ON CATEGORIA
FOR EACH ROW
BEGIN
  IF :NEW.ID_CAT IS NULL THEN
    SELECT SEQ_CATEGORIA.NEXTVAL INTO :NEW.ID_CAT FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_ESTADO_ID
BEFORE INSERT ON ESTADO
FOR EACH ROW
BEGIN
  IF :NEW.ID_EST IS NULL THEN
    SELECT SEQ_ESTADO.NEXTVAL INTO :NEW.ID_EST FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_USUARIO_ID
BEFORE INSERT ON USUARIO
FOR EACH ROW
BEGIN
  IF :NEW.ID_USR IS NULL THEN
    SELECT SEQ_USUARIO.NEXTVAL INTO :NEW.ID_USR FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_ARTICULO_ID
BEFORE INSERT ON ARTICULO
FOR EACH ROW
BEGIN
  IF :NEW.ID_ART IS NULL THEN
    SELECT SEQ_ARTICULO.NEXTVAL INTO :NEW.ID_ART FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_PRESTAMO_ID
BEFORE INSERT ON PRESTAMO
FOR EACH ROW
BEGIN
  IF :NEW.ID_PRES IS NULL THEN
    SELECT SEQ_PRESTAMO.NEXTVAL INTO :NEW.ID_PRES FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_MOVIMIENTO_ID
BEFORE INSERT ON MOVIMIENTO
FOR EACH ROW
BEGIN
  IF :NEW.ID_MOV IS NULL THEN
    SELECT SEQ_MOVIMIENTO.NEXTVAL INTO :NEW.ID_MOV FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_MANTENIMIENTO_ID
BEFORE INSERT ON MANTENIMIENTO
FOR EACH ROW
BEGIN
  IF :NEW.ID_MAN IS NULL THEN
    SELECT SEQ_MANTENIMIENTO.NEXTVAL INTO :NEW.ID_MAN FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_NOTIFICACION_ID
BEFORE INSERT ON NOTIFICACION
FOR EACH ROW
BEGIN
  IF :NEW.ID_NOT IS NULL THEN
    SELECT SEQ_NOTIFICACION.NEXTVAL INTO :NEW.ID_NOT FROM DUAL;
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_AUDITORIA_ID
BEFORE INSERT ON AUDITORIA
FOR EACH ROW
BEGIN
  IF :NEW.ID_AUD IS NULL THEN
    SELECT SEQ_AUDITORIA.NEXTVAL INTO :NEW.ID_AUD FROM DUAL;
  END IF;
END;
/

-- ---------------------------------------------------------
-- TRIGGERS DE AUDITORIA
-- ---------------------------------------------------------
CREATE OR REPLACE TRIGGER TRG_AUDIT_ARTICULO
AFTER INSERT OR UPDATE OR DELETE ON ARTICULO
FOR EACH ROW
DECLARE
  v_accion VARCHAR2(20);
  v_desc VARCHAR2(1000);
BEGIN
  IF INSERTING THEN
    v_accion := 'INSERT';
    v_desc := 'Artículo creado: ' || :NEW.NOM_ART || ' (Stock: ' || :NEW.CAN_ART || ')';
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('ARTICULO', TO_CHAR(:NEW.ID_ART), v_accion, 'SISTEMA', SYSDATE, v_desc);
  ELSIF UPDATING THEN
    v_accion := 'UPDATE';
    v_desc := 'Artículo modificado: ' || :OLD.NOM_ART || ' -> ' || :NEW.NOM_ART || ' (Stock: ' || :OLD.CAN_ART || ' -> ' || :NEW.CAN_ART || ')';
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('ARTICULO', TO_CHAR(:NEW.ID_ART), v_accion, 'SISTEMA', SYSDATE, v_desc);
  ELSIF DELETING THEN
    v_accion := 'DELETE';
    v_desc := 'Artículo eliminado: ' || :OLD.NOM_ART;
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('ARTICULO', TO_CHAR(:OLD.ID_ART), v_accion, 'SISTEMA', SYSDATE, v_desc);
  END IF;
END;
/

CREATE OR REPLACE TRIGGER TRG_AUDIT_PRESTAMO
AFTER INSERT OR UPDATE OR DELETE ON PRESTAMO
FOR EACH ROW
DECLARE
  v_accion VARCHAR2(20);
  v_desc VARCHAR2(1000);
BEGIN
  IF INSERTING THEN
    v_accion := 'INSERT';
    v_desc := 'Préstamo creado para usuario ID: ' || :NEW.ID_USR;
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('PRESTAMO', TO_CHAR(:NEW.ID_PRES), v_accion, 'SISTEMA', SYSDATE, v_desc);
  ELSIF UPDATING THEN
    v_accion := 'UPDATE';
    v_desc := 'Préstamo actualizado: Estado ' || :OLD.EST_PRES || ' -> ' || :NEW.EST_PRES || ', Estado Interno ' || :OLD.ID_EST || ' -> ' || :NEW.ID_EST;
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('PRESTAMO', TO_CHAR(:NEW.ID_PRES), v_accion, 'SISTEMA', SYSDATE, v_desc);
  ELSIF DELETING THEN
    v_accion := 'DELETE';
    v_desc := 'Préstamo eliminado físicamente ID: ' || :OLD.ID_PRES;
    INSERT INTO AUDITORIA (NOM_TABLA, ID_REGISTRO, NOM_ACCION, NOM_USUARIO, FEC_AUD, DES_AUD)
    VALUES ('PRESTAMO', TO_CHAR(:OLD.ID_PRES), v_accion, 'SISTEMA', SYSDATE, v_desc);
  END IF;
END;
/

-- ---------------------------------------------------------
-- PROCEDIMIENTOS ALMACENADOS (Stored Procedures)
-- ---------------------------------------------------------
CREATE OR REPLACE PROCEDURE registrar_prestamo(
  p_id_usr IN NUMBER,
  p_id_est IN NUMBER,
  p_obs_pres IN VARCHAR2,
  p_id_pres OUT NUMBER
) AS
BEGIN
  SELECT SEQ_PRESTAMO.NEXTVAL INTO p_id_pres FROM DUAL;
  
  INSERT INTO PRESTAMO (ID_PRES, FEC_PRES, FEC_ENTREGA, OBS_PRES, EST_PRES, ID_USR, ID_EST)
  VALUES (p_id_pres, SYSDATE, SYSDATE, p_obs_pres, 1, p_id_usr, p_id_est);
END;
/

CREATE OR REPLACE PROCEDURE registrar_devolucion(
  p_id_pres IN NUMBER,
  p_obs_pres IN VARCHAR2
) AS
BEGIN
  UPDATE PRESTAMO
  SET FEC_DEVOLUCION = SYSDATE,
      OBS_PRES = p_obs_pres,
      ID_EST = (SELECT ID_EST FROM ESTADO WHERE NOM_EST = 'DISPONIBLE' AND TIPO_EST = 'ARTICULO')
  WHERE ID_PRES = p_id_pres;
END;
/
